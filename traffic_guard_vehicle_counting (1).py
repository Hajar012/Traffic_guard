...
VIDEOS = {
    "North": "trafficlight/south-lane (2).mp4",
    "East": "trafficlight/north-lane.mp4",
    "West": "trafficlight/east-lane.mp4",
    "South": "trafficlight/east-lane.mp4",
}
...
# Scale factor for the dashboard window (e.g., 0.6 makes it 60% of original size)
WINDOW_SCALE = 0.6
...
def run_all_in_one(video_map, model_path):
    ...
    while True:
        ...
        # Resize window if scaling is requested
        if 'WINDOW_SCALE' in globals() and WINDOW_SCALE and WINDOW_SCALE != 1.0:
            h, w = dashboard.shape[:2]
            new_w = max(1, int(w * WINDOW_SCALE))
            new_h = max(1, int(h * WINDOW_SCALE))
            dashboard = cv2.resize(dashboard, (new_w, new_h), interpolation=cv2.INTER_AREA)
        cv2.imshow("MASTER - Traffic Control", dashboard)
...
def run_edge_only(road, video_path, master_ip, model_path, show_edge=False):
    ...
    while True:
        ...
        if show_edge:
            cv2.imshow(f"EDGE - {road}", annotated)
...
if __name__ == "__main__":
    ...
    if args.mode == "all":
        ...
        video_map = {
            ...
            "West": args.west_video,
            "South": args.south_video,
            "East": args.east_video,
        }
